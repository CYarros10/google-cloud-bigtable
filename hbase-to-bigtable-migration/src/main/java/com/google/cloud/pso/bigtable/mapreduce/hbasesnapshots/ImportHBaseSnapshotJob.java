 ...
 */
package com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots;

import static com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots.ImportJobCommon.DEST_HBASE_ZK_HOST_KEY;
import static com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots.ImportJobCommon.IMPORT_SNAPSHOT_JOBNAME_KEY;
import static com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots.ImportJobCommon.SNAPSHOTNAME_KEY;
import static com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots.ImportJobCommon.SNAPSHOT_RESTOREDIR_KEY;
import static com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots.ImportJobCommon.SNAPSHOT_SPLITS_PER_REGION_DEFAULT;
import static com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots.ImportJobCommon.SNAPSHOT_SPLITS_PER_REGION_KEY;
import static com.google.cloud.pso.bigtable.mapreduce.hbasesnapshots.ImportJobCommon.TABLENAME_KEY;
...
public class ImportHBaseSnapshotJob extends Configured implements Tool {
  ...
  private static void usage(final String errorMsg) {
    ...
    System.out.println(
        "Usage: hadoop jar <JAR> <CLASS> <properties> <snapshot-name> <snapshot-dir> <tablename> <temp-dir>\n"
            ...
            + "=<zk-quorum:zk-quorum-port:zk-hbase-root>  (ie. hbase-master-1:2181/hbase\n"
            + " or \n"
            + "  -D"
  ...
  protected static int setConfFromArgs(Configuration conf, String[] args) {
    ...
    // set hbase destination zk cluster key (zk host:zk port:/hbase)
    if (args.length > 5) {
      conf.set(DEST_HBASE_ZK_HOST_KEY, args[5]);
    }
    ...
    // setup implicit bigtable configs only if properties are set
    if (null != conf.get(BigtableOptionsFactory.PROJECT_ID_KEY)) {
  ...
  protected static class SnapshotMapper extends TableMapper<ImmutableBytesWritable, Put> {
    ...
    private ImmutableBytesWritable rowKeyTransformed = new ImmutableBytesWritable();
     ...
     * tombstones (see {@link Scan#setRaw(boolean)} if required).
     * Additionally @see <a
    ...
    protected void map(ImmutableBytesWritable key, Result result, Context ctx)
      ...
      // prefixes the original key with a byte array representation of an integer
      byte[] rowKey = keyTransform(key);
      ...
      while (cellIt.hasNext()) {
        ...
        for (int i = 0; i < MAX_CELLS && cellIt.hasNext(); i++) {
          ...
          Cell formattedCell = CellUtil.createCell(
                  rowKey,
     ...
     * @return
     */
    protected byte[] keyTransform(ImmutableBytesWritable currentKey) {
      return ByteBuffer.allocate(currentKey.get().length)
              .put(currentKey.get())
              .array();
    }
